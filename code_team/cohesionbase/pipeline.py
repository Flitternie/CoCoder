"""CohesionPipeline — load data → build workspace → orchestrate multi-agent run.

Same structure as depgraphbase.pipeline but uses cohesionbase prompts and orchestrator.
"""
...
from cohesionbase.prompts import build_leader_user_prompt
...
class CohesionPipeline:
    """Cohesion-based multi-agent pipeline."""
    ...
    def create_run_config(
        ...
        if gt_rib:
            ...
            rib_tag = "ribgensim"
        run_id = f"{timestamp}_{model_slug}_cohesionbase_{rib_tag}_{dataset_slug}_{repo_slug}"
    ...
    def run(self) -> None:
        """Execute the cohesionbase pipeline."""
        start = time.time()
        ...
        if enable_tracking:
            from dashboard.cohesionbase.parallel_progress import CohesionProgressTracker
            tracker = CohesionProgressTracker(
                self.run_dir, self.cfg.run_id,
                workspace_dir=self.cfg.workspace_dir,
            )
            tracker.start()

        # Create orchestrator
        from cohesionbase.orchestrator import CohesionOrchestrator

        orchestrator = CohesionOrchestrator(
            workspace_dir=workspace_dir,
        ...
        try:
            ...
            if setup_script:
                ...
                if script_path.exists():
                    print(f"[cohesionbase] Running setup script: {setup_script}")
                    subprocess.run(
            ...
            if rib_provided:
                print(f"[cohesionbase] Using ground-truth RIB: {resolve_gt_rib(self.cfg, 'rib_dep_ground_true.json')}")

            # Build the initial prompt and start
    ...
    def _build_initial_prompt(self, bundle: RepoBundle, workspace_dir: Path, *, rib_provided: bool = False) -> str:
        return build_leader_user_prompt(bundle, rib_provided=rib_provided)
