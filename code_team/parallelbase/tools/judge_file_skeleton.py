"""judge_file_skeleton tool — per-file skeleton quality evaluator.

Evaluates a single file's skeleton code against its SSAT architecture entry.
Used by Subagents to self-evaluate their skeleton before proceeding to code.

Based on JudgeSkeletonTool but operates on a single file, not the full project.
"""
...
from common.utils.ssat_helpers import load_architecture, find_file_item
...
def file_skeleton_judge_prompt(file_architecture: dict, skeleton_code: str, file_path: str) -> str:
    """Prompt for evaluating a single file's skeleton against its SSAT entry."""
    return f"""You are an expert software architecture reviewer.
You will be given two inputs:

1. **File Architecture Specification (ARCH)** - the SSAT entry for a single file, including its intended classes, functions, parameters, and descriptions.
...
class JudgeFileSkeletonAction(Action):
...
class JudgeFileSkeletonObservation(Observation):
    score: int = 0
    ...
    def to_llm_content(self) -> Sequence[TextContent | ImageContent]:
        if self.error:
            return [TextContent(text=f"judge_file_skeleton FAILED: {self.error}")]
        ...
        if self.score < 8:
            text += (
                ...
                "then call judge_file_skeleton again. "
...
class JudgeFileSkeletonExecutor(ToolExecutor[JudgeFileSkeletonAction, JudgeFileSkeletonObservation]):
    ...
    def _completion_with_timeout(self, prompt: str):
        ...
        executor = ThreadPoolExecutor(max_workers=1, thread_name_prefix="judge_file_skeleton")
    ...
    def __call__(
        self,
        action: JudgeFileSkeletonAction,
        conversation=None,
    ) -> JudgeFileSkeletonObservation:
        ...
        try:
            ...
            return JudgeFileSkeletonObservation(
                error=str(e), target_file=action.target_file,
            )

        # Find the target file's SSAT entry
        file_item = find_file_item(arch_data, action.target_file)
        if not file_item:
            return JudgeFileSkeletonObservation(
                error=f"File '{action.target_file}' not found in SSAT architecture",
        ...
        prompt = file_skeleton_judge_prompt(
...
_DESCRIPTION = """\
Evaluate a single file's skeleton code against its SSAT architecture entry.

Call this after generating your skeleton to check quality. The tool reads
your file's SSAT entry and skeleton code, then scores on interface matching,
...
class JudgeFileSkeletonTool(ToolDefinition[JudgeFileSkeletonAction, JudgeFileSkeletonObservation]):
    ...
    def create(cls, conv_state, llm=None, workspace_dir=None) -> Sequence[ToolDefinition]:
        if llm is None or workspace_dir is None:
            raise ValueError("JudgeFileSkeletonTool requires llm and workspace_dir")
        executor = JudgeFileSkeletonExecutor(llm, workspace_dir)
        return [cls(
            description=_DESCRIPTION,
            action_type=JudgeFileSkeletonAction,
            observation_type=JudgeFileSkeletonObservation,
            executor=executor,
        )]
